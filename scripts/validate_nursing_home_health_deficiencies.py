"""Validate the CMS nursing home Health Deficiencies source file.
...
def harm_group(scope_severity: str) -> str:
    code = str(scope_severity or "").strip().upper()
    if code in {"J", "K", "L"}:
        return "immediate_jeopardy"
    if code in {"G", "H", "I"}:
        return "actual_harm_not_immediate_jeopardy"
    if code in {"D", "E", "F"}:
        return "no_actual_harm_potential_more_than_minimal"
    if code in {"A", "B", "C"}:
        return "no_actual_harm_potential_minimal"
    if not code:
        return "missing"
...
def main() -> None:
    source_path = find_latest(HEALTH_PATTERN)
    ...
    with source_path.open("r", encoding="utf-8-sig", newline="") as handle:
        ...
        for row in reader:
            ...
            harm_counts[harm_group(scope)] += 1
    ...
    print(f"Joined CT health deficiency CCNs to current staffing: {len(joined_ccns)}")
    print(f"Unmatched CT health deficiency CCNs: {len(unmatched_ccns)}")
    ...
    for scope, count in sorted(scope_counts.items()):
        print(f"  {scope}: {count}")
    print("Conservative harm/IJ grouping from structured scope/severity:")
    for group, count in sorted(harm_counts.items()):
        print(f"  {group}: {count}")
    ...
    if invalid_survey_dates or malformed_tag_count or citation_lookup_miss_count:
        raise SystemExit(1)
