(function () {
  ...
  function getNorsSourcePackRows(data) {
    return data?.norsSourcePackLock?.source_pack_rows || [];
  }

  function getNorsSourcePriorityRules(data) {
    return data?.norsSourcePackLock?.source_priority_rules || [];
  }
  ...
  function getKnowledgeHumanReviewWarnings({ norsCode, topicMatches = [], resources = [], keywordText = '' } = {}, data) {
    ...
    getKnowledgeHumanReviewFlags(data).forEach(flag => {
      ...
      if (id === 'HRF-B14-001' && (
        matchedCodes.includes('F02') ||
        /call light|call bell|requests? for assistance|waited .*help|no one came/.test(`${matchedTopics} ${text}`)
      )) {
        warnings.push(`Authority specificity note (${id}): ${action}`);
      }
      if (id === 'HRF-B14-002' && (
        matchedCodes.some(code => ['F02', 'F08', 'A05'].includes(code)) ||
  ...
  window.Crosswalk = {
    ...
    getNorsSourcePackRows,
    getNorsSourcePriorityRules,
