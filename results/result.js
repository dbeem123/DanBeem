...
(() => {
  ...
  const apiOrigin = 'https://api.danbeem.xyz';
  const resultPath = '/v1/results/' + encodeURIComponent(id);
  ...
  function protectedResultUrl(value, endpoint) {
    if (typeof value !== 'string' || !value || value.startsWith('//')) return null;
    try {
      const url = new URL(value, apiOrigin);
      if (url.origin !== apiOrigin || url.protocol !== 'https:' || url.username || url.password || url.hash) return null;
      if (endpoint) {
        if (url.pathname !== resultPath + '/' + endpoint) return null;
      } else if (url.pathname !== resultPath && !url.pathname.startsWith(resultPath + '/')) return null;
      return url.href;
    } catch { return null; }
  }
  ...
  function render(item) {
    ...
    const fileUrl = protectedResultUrl(item.media.url, 'media');
    ...
    const downloadUrl = protectedResultUrl(item.media.download_url, 'download');
    download.hidden = !downloadUrl;
    if (downloadUrl) download.href = downloadUrl;
    const shareUrl = item.sharing && item.sharing.enabled === true ? protectedResultUrl(item.sharing.url) : null;
  ...
  async function load() {
    ...
    try {
      const response = await fetch(apiOrigin + resultPath, {
        method: 'GET', mode: 'cors', credentials: 'include', cache: 'no-store', redirect: 'error',
